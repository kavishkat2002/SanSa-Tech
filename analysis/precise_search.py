...
def get_order(stats_list, s_off, h_off, s_r, m_r, h_r, t_f):
    ...
    # Matching race_simulator.py sort exactly (deterministically)
    # The actual engine uses: (round(d.total_time, 10), d.grid_pos)
...
def run():
    ...
    SOFT_BASES   = [-3.5, -2.8, -2.5, -2.0, -1.8, -1.5, -1.2, -1.0, -0.8]
    HARD_BASES   = [0.8, 1.0, 1.2, 1.5, 1.8, 2.0, 2.5, 2.8, 3.2, 3.5]
    DEG_SOFTS    = [0.03, 0.035, 0.04, 0.045, 0.05, 0.06]
    DEG_MEDS     = [0.014, 0.015, 0.018, 0.02, 0.022, 0.025]
    DEG_HARDS    = [0.005, 0.007, 0.008, 0.009, 0.01, 0.012]
    TEMP_FACTORS = [0.0, 0.005, 0.008, 0.01, 0.012, 0.015, 0.018, 0.02]
    p = 2.0

    print("🚀 Precise search around core values...")
    for p in [1.0, 2.0]:
        ...
        for s_off, h_off, s_r, m_r, h_r, t_f in product(SOFT_BASES, HARD_BASES, DEG_SOFTS, DEG_MEDS, DEG_HARDS, TEMP_FACTORS):
            if get_order(race_stats[0], s_off, h_off, s_r, m_r, h_r, t_f) == targets[0]:
                ...
                for i in range(1, 100):
                    if get_order(race_stats[i], s_off, h_off, s_r, m_r, h_r, t_f) == targets[i]:
                        score += 1
                    else: break
                
                if score > 1:
                    print(f"  ✓ Score {score}/100: s_off={s_off}, h_off={h_off}, s_r={s_r}, m_r={m_r}, h_r={h_r}, t_f={t_f}, p={p}")
                    if score == 100:
                        print("\n🎉 PERFECT MATCH FOUND!")
                        return
